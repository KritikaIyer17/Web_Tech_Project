...
function saveMemberInfo(event) {
  ...
  const keys = ['reg_no', 'owner_name', 'd_o_a', 'slot']
  const obj = {}
  ...
  members.forEach((item) => {
    if (obj.slot === item.slot) {
      if (obj.d_o_a === item.d_o_a) {
        alert("Unable to allocate slot. Selected slot is unavailabe on the selected day.");
...
function updateMemberData() {
  if ($('#edit_reg_no').val() == '' || $('#edit_owner_name').val() == '' || $('#edit_d_o_a').val() == '' || $('#edit_slot').val() == '') {
